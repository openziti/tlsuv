...
TEST_CASE("deflate", "[http]") {
  ...
  0x24, 0x59, 0x49, 0xc2
    ...
    um_inflate(inflater, (const char*)packet_bytes, sizeof(packet_bytes));
    ...
    um_inflate(inflater, (const char*)packet_bytes, inputLen / 2 );
    um_inflate(inflater, (const char*)packet_bytes + (inputLen/2), inputLen - inputLen / 2 );
    CHECK(um_inflate_state(inflater) == 1);
    CHECK_THAT(res.str, Catch::Contains(R"("method": "GET")"));
    um_free_inflater(inflater);
